...
impl DashboardService {
    ...
    pub fn start_periodic_updates(self: Arc<Self>) {
        let ws_sender = self.ws_tx.clone();
        tokio::spawn(async move {
            let mut interval = tokio::time::interval(Duration::from_secs(10));
            let force_fetch_every = self.config().force_fetch_interval_secs / 10;
            let mut cycle: u64 = 0;
            loop {
                interval.tick().await;
                cycle += 1;
                ...
                match self.get_dashboard_with_refresh(force).await {
                    Ok(dashboard_state) => {
                        let cfg = self.config();
                        let solar_watts = self.parse_solar_watts(&dashboard_state);
                        let chart_points = self.compute_solar_chart().await;
                        let charger_state = self.compute_car_state(&dashboard_state).await;
                        ...
                        let solar_return_vals: Vec<String> = buffer_chart_points
                            ...
                            .map(|v| format!("{:.1}", v))
                            .collect();
                        let pump_status = self.compute_pump_status(&dashboard_state);
