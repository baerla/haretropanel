(function () {
    ...
    var FL = {
        ...
        log: function (tag, msg) {
            ...
            var level = 'log';
            this.entries.push({ time: time, tag: tag, msg: msg, level: level });
            this.flush();
        ...
        info: function (tag, msg) {
            var now = new Date();
            var time = now.toTimeString().slice(0, 8) + '.' + String(now.getMilliseconds()).padStart(3, '0');
            this.entries.push({ time: time, tag: tag, msg: msg, level: 'info' });
            this.flush();
        ...
        warn: function (tag, msg) {
            var now = new Date();
            var time = now.toTimeString().slice(0, 8) + '.' + String(now.getMilliseconds()).padStart(3, '0');
            this.entries.push({ time: time, tag: tag, msg: msg, level: 'warn' });
            this.flush();
        ...
        error: function (tag, msg) {
            var now = new Date();
            var time = now.toTimeString().slice(0, 8) + '.' + String(now.getMilliseconds()).padStart(3, '0');
            this.entries.push({ time: time, tag: tag, msg: msg, level: 'error' });
            this.flush();
        ...
        success: function (tag, msg) {
            var now = new Date();
            var time = now.toTimeString().slice(0, 8) + '.' + String(now.getMilliseconds()).padStart(3, '0');
            this.entries.push({ time: time, tag: tag, msg: msg, level: 'success' });
            this.flush();
        ...
        render: function () {
            ...
            for (var i = 0; i < this.entries.length; i++) {
                ...
                html += '<div class="debug-log-entry ' + e.level + '">'
                    + '<span class="debug-log-time">' + e.time + '</span>'
                    + '<span class="debug-log-tag">[' + e.tag + ']</span>'
                    + e.msg + '</div>';
            }
            body.innerHTML = html;
        ...
        consoleLog: function (level, tag, msg) {
            if (level === 'log') console.log('[' + tag + '] ' + msg);
            else if (level === 'info') console.info('[' + tag + '] ' + msg);
    ...
    document.querySelectorAll('.garage-card button').forEach(function(el) {
        el.addEventListener('click', function() {
            ...
            wsSend({action:'toggle', entity_id: el.dataset.entityId});
        });
    });
    ...
    document.querySelectorAll('.toggle-btn, .script-btn').forEach(function(btn) {
        ...
        if (entity) {
            ...
            btn.addEventListener('click', function() {
                ...
                wsSend({action: action, entity_id: entity});
            });
        }
    });
